...
export async function generateMetadata({ params }: PageProps) {
  ...
  return buildMetadata({
    ...
    // Leave image undefined unless the study sets one: buildMetadata then bakes
    // the title AND description into the default /api/og card.
    image: study.frontmatter.image,
    type: "article",
    publishedTime: study.frontmatter.date,
    modifiedTime: study.frontmatter.updated ?? study.frontmatter.date,
    authors: study.frontmatter.author ? [study.frontmatter.author] : undefined,
...
export default async function CaseStudyPage({ params }: PageProps) {
  ...
  const { title, description, date, updated, author, tags } = study.frontmatter;
      ...
      <article className="surface-cream pb-16 pt-28 sm:pt-32 lg:pt-36">
        <JsonLd
          data={breadcrumbSchema([
            ...
            { name: title, href: `/case-studies/${study.slug}` },
          ])}
        />
        <JsonLd
          data={articleSchema({
